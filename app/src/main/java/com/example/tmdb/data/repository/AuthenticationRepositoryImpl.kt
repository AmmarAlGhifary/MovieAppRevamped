...
import com.example.tmdb.data.mapper.toAccessToken
...
import com.example.tmdb.domain.model.AccessToken
...
class AuthenticationRepositoryImpl @Inject constructor(
    ...
    private val authHeader = BuildConfig.API_KEY

    override suspend fun createRequestToken(redirectTo: String): Resource<RequestToken> =
        safeApiCall.execute {
    ...
    override suspend fun validateRequestToken(
        ...
        password: String,
        requestToken: String
    ): Resource<RequestToken> = safeApiCall.execute {
    ...
    override suspend fun createSession(accessToken: String): Resource<Session> =
        safeApiCall.execute {
            api.createSession(authHeader, mapOf("access_token" to accessToken)).toSession()
        }
}
